  ...
  <el-main class="SmallBox">
    <div v-show="modular_1">
      <div class="header">
        ...
        <div class="Range">
          <p class="text_1">选择练习{{SelectSystem=='问诊实训'?'知识点/类似症/相关症':'知识点'}}<span>*不勾选则视为全选</span></p>
          <div class="knowledge">
    ...
    clickSetUp() { // 设置新的练习题
      ...
      if (that.SelectSystem == '原文实训') {
        ...
        if (that.SelectSystem == '案例实训') {
          ...
          url = that.$store.state.Q_http + 'interroExamination/queryInterrogationExamQuestionClassRoomBefore'
        }
        that.$axios({
          url: url,
          method: 'post',
